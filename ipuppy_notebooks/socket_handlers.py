...
async def execute_shell_command(command: str) -> dict:
    ...
    try:
        ...
        if len(args) >= 2 and args[0] == "pip" and args[1] == "install":
            ...
            if env_type == "uv":
                warning_msg = "🐶 Woof! It looks like you're in a uv-managed environment.\n"
...
async def execute_code_with_streaming(cell_index: int, code: str):
    ...
    try:
        ...
        if len(lines) == 1 and lines[0].strip().startswith('!'):
            ...
            shell_command = lines[0].strip()[1:].strip()  # Remove the ! and any whitespace
...
async def execute_python_code_streaming(cell_index: int, code: str):
    ...
    try:
            ...
            try:
                ...
                if msg['parent_header'].get('msg_id') == msg_id:
                    ...
                    if msg_type == 'status' and content['execution_state'] == 'idle':
                        logger.info(f"Python code execution completed for cell {cell_index}")
                        break
        ...
        try:
            ...
            logger.error(f"Failed to broadcast Python execution error: {broadcast_error}")
...
def get_file_completions(partial_path: str) -> list:
    ...
    try:
        ...
        if 'original_path' in locals() and (original_path == '~/' or original_path == '~'):
